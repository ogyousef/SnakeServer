...
class SnakeGame:
    ...
    def check_collision(self, user_id) : 
        ...
        for other_id, snake in self.players.items():
            if other_id != user_id:
                if self.players[user_id].head.pos in list(map(lambda z: z.pos, snake.body)):
                    return True
    ...
    def get_state(self) : 
        ...
        # print(f"players_pos_str {players_pos_str}")
        snacks_pos = "**".join([str(s.pos) for s in self.snacks])
        # print(f"players: {players_pos_str} |Snacks: {snacks_pos}")
